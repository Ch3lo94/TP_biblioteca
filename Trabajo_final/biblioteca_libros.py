...
class Libros:
    ...
    def guardar_libros(self, ruta):
        ...
        with open(ruta, 'w', encoding='utf-8') as archivo:
            # Escribir esa información usando la clase creada en Libro (se usa indent para que sea más legible a la anidación con 4 espacios)
        ...
        while True:
            ...
            for libro in self.libros:
                if libro.id == id_libro:
                    encontrado = True
                    limpiar_consola()
                    print(f"¿Desea editar el libro '{libro.titulo}'?")
                    print("     0. Cancelar")
                    print("     1. Confirmar")
                    confirmacion = input("Ingrese el número de la opción que desea realizar: ")
                    limpiar_consola()
    ...
    def buscar_libros(self):
        while True:    
            ...
            limpiar_consola()
            print(f'''             Biblioteca - Buscar libros
                    1) Nombre de Libro
                    2) Genero
                    3) Autor
                    4) Año
            ...
            match opc:
                case "1":
                    valor = self._validar_input("Ingrese el Nombre de Libro")
                    resultados = [lib for lib in self.libros if valor.lower() in lib.titulo.lower()]
                case "2":
                    valor = self._validar_input("Ingrese el Genero")
                    resultados = [lib for lib in self.libros if valor.lower() in lib.genero.lower()]
                case "3":
                    valor = self._validar_input("Ingrese el Autor")
                    resultados = [lib for lib in self.libros if valor.lower() in lib.autor.lower()]
                case "4":
                    valor = self._validar_input("Ingrese el Año")
                    resultados = [lib for lib in self.libros if valor in lib.anio]
                case "5":
                    valor = self._validar_input("Ingrese el Editorial")
                    resultados = [lib for lib in self.libros if valor.lower() in lib.editorial.lower()]
                case "0":
                    print("Saliendo de la búsqueda. Presione Enter para continuar.")
                    input()
                    return
                case _:
                    print(f"Opción '{opc}' no válida. Presione Enter para continuar.")
                    input()
                    continue

            limpiar_consola()
            if resultados:
                print(f"\nSe encontraron {len(resultados)} libro(s) que coinciden con el criterio seleccionado:\n")
                print(f"  {'ID':<5} | {'Titulo':^25} | {'Autor':^30} | {'Editorial':^20} | {'Año':^8} | {'Género':^10} | {'Disponibles':^10}")
                print('-' * 130)
                for lib in resultados:
                    print(f"  {lib.id:<5} | {lib.titulo:^25} | {lib.autor:^30} | {lib.editorial:^20} | {lib.anio:^8} | {lib.genero:^10} | {lib.cantidad_disp:^10}")
            else:
                print(f"No se encontraron libros para '{valor}'.")
            print ("")
            input("Presione enter para continuar.")
    ...
    def menu(self):
        ...
        while continuar:
            ...
            print(f'''             Biblioteca
                   ...
                   {Libros.BUSCAR_LIBRO}) Buscar Libros
                   {Libros.SALIR} ) Salir ''')
            opc = input("Selecciona una Opción: ")
            try:
                opc = int(opc)

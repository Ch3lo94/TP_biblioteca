...
class Prestamos:
    ...
    def imprimir_prestados(self):
        print(f"{'ID Préstamo':<12} {'Libro':<25} {'Socio':<25} {'Fecha de Préstamo':<15}")
        print('-' * 80)
        for prestamo in self.prestamos:
            if prestamo.estado_prestamo == 'Prestado':
                ...
                print(f"{prestamo.id_prestamo:<12} {nombre_libro:<25} {nombre_socio:<25} {prestamo.fecha_prestamo:<15}")
        print("")
                ...
                # Verificar si el libro ya está prestado al socio
                    for prestamo in self._prestamos:
                        ...
                        if libro.cantidad_disp == "0":
                            ...
                            continue  # Volver a solicitar el ID del libro
                    else:
                        self._ultimo_id += 1
    ...
    def registrar_devolucion(self):
        while True:
            ...
            if prestamo.estado_prestamo == "Devuelto":
                print("Este préstamo ya ha sido devuelto.")
                input("Presione enter para continuar.")
                return
    ...
    def reporte_prestamos(self):
        ...
        if not self._prestamos:
            ...
            print(f"{'ID Préstamo':<12} {'ID Socio':<10} {'ID Libro':<10} {'Fecha Préstamo':<15} {'Fecha Devolución':<18} {'Estado':<10}")
            print('-' * 75)
            for prestamo in self._prestamos:
                print(f"{prestamo.id_prestamo:<12} {prestamo.id_socio:<10} {prestamo.id_libro:<10} {prestamo.fecha_prestamo:<15} {prestamo.fecha_devolucion:<18} {prestamo.estado_prestamo:<10}")
        input("Presione enter para continuar.")
